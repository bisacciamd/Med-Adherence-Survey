...
```{r adherence-lang}
kable(adherence_by_country[order(adherence_by_country$n_total, decreasing = TRUE), ], caption = "Adherence by country", align = "c", col.names = c("Language", "Respondents (n)", "Adherent (n)", "Adherent (%)"))
```
...
```{r}
library(knitr)
summary_stats |> select(Language, n_total, adherent, forget_lt_week, forget_gt_week, stopped, impact_identity, vouchers, fine, support_program, phone_reminders, emoji, pension_collection, health_info)
